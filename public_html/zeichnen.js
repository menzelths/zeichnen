...
$(function(){
$("body").html("<div id='rahmen' style='position: relative;'> <canvas id='layer0' width='800' height='600' style='z-index: 0;'></canvas> <canvas id='layer1' width='800' height='600' style='position: absolute; left: 0; top: 0; z-index: 1;'></canvas><canvas id='layer2' width='800' height='600' style='position: absolute; left: 0; top: 0; z-index: 2;'></canvas></div><div id='knoepfe'> <input type='button' class='holen knopf' b='Stift' id='stift' value='Stift'/><input type='button' class='holen knopf' b='Radierer' id='radierer' value='Radierer'/><input type='button' class='holen knopf' b='Raster ändern' id='raster' value='Raster an / aus' /><input type='button' class='holen knopf' b='SVG erzeugen' id='knopf' value='SVG anzeigen' /><br><input type='button' class='holen knopf' b='Alles löschen' id='loeschen' value='Alles löschen' /></div><div id='ausgabe'></div><div id='info' ></div>");
$("body").append("<div id='bild'><svg width='800' height='600' id='svgbild'></svg></div>");
var c=$("#layer2")[0];
var breite=800;
var hoehe=600;
var toleranz_steigung=0.03;
var rx=5; // zum radieren in rechtecke zerlegen
var ry=5; 
...
c.ontouchstart=function (event) {event.preventDefault();daten=[];gedrueckt=true;touch=true; maus=false;touchBewegt(event); };
c.ontouchmove=function (event) {event.preventDefault();if (touch===true) touchBewegt(event);};
c.ontouchend=function (event) {event.preventDefault();if (touch===true){berechnung=true;gedrueckt=false; touchBewegt(event);}};
c.onmousedown=function (event) {event.preventDefault(); daten=[];gedrueckt=true;maus=true;touch=false;mausBewegt(event);};
c.onmousemove=function (event) {event.preventDefault();if (maus===true){mausBewegt(event);}};
c.onmouseup=function (event) {event.preventDefault();if (maus===true){berechnung=true;gedrueckt=false;mausBewegt(event);}};
...
function zeichneZufaellig(n){ // zeichne n zufällige bezierkurven
  ...
  kurven.push([k,Bezier.findeRechtecksHuelle(k),0]);
  }
}
...
function zeichneOderRadiere(){
  if (berechnung===true){
          ...
          if (tk.length>0){
           kurven.splice(betroffeneKurven[i],1);
            for (var l=0;l<tk.length;l++){
             kurven.splice(betroffeneKurven[i],0,[tk[l],Bezier.findeRechtecksHuelle(tk[l]),1]); 
            }
          }
    ...
    daten=[];
    var leer=gc.createImageData(800,600);
    for (var i=0;i<leer.length;i+=4){
      leer[i]=255;
      leer[i+1]=255;
      leer[i+2]=255;
        ...
        for (var i=0;i<erg.length;i++){
         var tmp=erg[i];
          if (tmp.length===1){ // punkt
            ...
            gc2.stroke();
            kurven.push([tmp,Bezier.findeRechtecksHuelle(tmp),0]); // alle zeichnungen speichern
          }
...
var Bezier={
  ...
  findeNullstelle: function(kontrollpunkte, punkt,u){
   var k1=[],k2=[];
    ...
    if (Math.abs(df)<this.TOLERANZ) return u;
